...
<%--String manager=(String)session.getAttribute("manger");--%>
<%--//验证用户是否登录--%>
...
<c:if test="${sessionScope.manger.name eq null or empty sessionScope.manger.name  }"><% response.sendRedirect("login.jsp");%></c:if>
  ...
  <tr>
    <td height="118" valign="top" background="Images/top_bg.gif" bgcolor="#EEEEEE"><table width="100%" height="33"  border="0" cellpadding="0" cellspacing="0">
      ...
      <table width="93%" height="79"  border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td height="69" align="right" valign="bottom">当前登录用户：${sessionScope.manger.name}</td>
        </tr>
    </table></td>
  </tr>
</table>
